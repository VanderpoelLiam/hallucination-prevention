...
def make_pretty(sent):
    wrapped = textwrap.fill(sent, 100)
...
def get_data():
    path = "data/xsum-hallucination-raw/"
    ref = path +"Gold.target"
    ref_label = path + "Gold.label"
    ...
    path = "data/xsum-hallucination/"
    target = path + "test.bpe.target"
...
if __name__ == '__main__':
    ...
    refs, ref_labels, targets = get_data()
    path = "data/xsum-hallucination/"

    # Automatically align as best we can
    # with open(path + "test.label", 'w') as fp:
    #     for i in range(500):
    #         if i % 10 == 0:
    #             print(i)
    ...
    count = 0
    with open(path + "test.label") as f:
        for line in f:
            if "?" in line:
                count += 1
    ...
    with open(path + "test.label") as f:
        for i, target_label in enumerate(f):
            ...
            if ("?" in target_label) or (not len_target == len_target_label):
